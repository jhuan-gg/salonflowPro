...
import { Download, BarChart3, TrendingUp, Users, Calculator, Calendar as CalendarIcon } from "lucide-react";
...
export default function Relatorios() {
  ...
  const receitaLiquida = totalGeral - comissaoGeral;
    ...
    <AppLayout>
      <div className="p-4 md:p-8 space-y-6 animate-fade-in font-sans pb-24 md:pb-8 text-foreground">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="bg-card border-l-4 border-l-blue-500/70">
            <CardContent className="p-4 md:p-6">
              <div className="flex justify-between items-start">
                <div className="space-y-1 w-full">
                  <p className="text-sm font-semibold text-foreground">Gasto Médio (Ticket)</p>
                  <p className="text-[10px] text-muted-foreground mb-1">(Média do que cada cliente paga)</p>
                  <p className="text-xl md:text-2xl font-bold text-blue-600 dark:text-blue-400 truncate">{formatCurrency(ticketMedio)} <span className="text-sm font-normal text-muted-foreground">/ {totalAtendimentos}x</span></p>
                </div>
              </div>
            </CardContent>
          </Card>
